...
class Acc31Connector @Inject()(httpClient: HttpClient,
  ...
  def retrieveCashTransactions(can: String,
                               from: LocalDate,
                               to: LocalDate): Future[Either[ErrorResponse, Option[CashTransactionsResponseDetail]]] = {
  ...
  private def cashAccountTransactions(resp: GetCashAccountTransactionListingResponse):
    ...
    resp.responseCommon match {
      case CashTransactionsResponseCommon(status@"OK", Some(msg), _, _, _) if msg.contains("025-No associated data found") =>
        log.info(message = s"$status: $msg")
        Left(NoAssociatedDataException)

      case CashTransactionsResponseCommon(status@"OK", Some(msg), _, _, _) if hasQueryExceededTheThreshold(msg) =>
        log.info(message = s"$status: $msg")
