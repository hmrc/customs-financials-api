...
package controllers {
  ...
  class ReverseAccountAuthoritiesController(_prefix: => String) {
    ...
    // @LINE:13
    def get(eori:models.EORI): Call = {
      
      Call("GET", _prefix + { _defaultPrefix } + "customs-financials-api/" + play.core.routing.dynamicString(implicitly[play.api.mvc.PathBindable[models.EORI]].unbind("eori", eori)) + "/account-authorities")
    }
