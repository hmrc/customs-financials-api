  ...
  "NotificationCache" should {
    ...
    "put and get notifications from mongo db" in new Setup {
      running(app) {
        ...
        val expectedNotifications =
          List(
            securityStmtNotification1,
            securityStmtNotification2,
            c79CertNotification1,
            c79CertNotification2,
            c79CertNotification3,
            c79CertNotification4,
            securityStmtNotification3,
            securityStmtNotification4,
            pvatStmtNotification1,
            pvatStmtNotification2,
            pvatStmtNotification3,
            pvatStmtNotification4
          )
    ...
    "delete requested C79Certificate by fileRole" in new Setup {
      ...
      running(app) {
        val result = await(for {
          _             <- cache.putNotifications(
                             NotificationsForEori(
                               Eori1,
                               Seq(requestedStatement1, requestedStatement2, nonRequestedStatement),
                               lastUpdated
                             )
                           )
