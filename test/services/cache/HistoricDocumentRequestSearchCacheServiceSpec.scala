...
class HistoricDocumentRequestSearchCacheServiceSpec extends SpecBase {
  ...
  "saveHistoricDocumentRequestSearch" should {
    ...
    "return true when document is insert successfully" in new Setup {
      ...
      service.saveHistoricDocumentRequestSearch(histDocRequestSearch).map { result =>
        result mustBe true
      }
    }
  ...
  "retrieveHistDocRequestSearchDocsForCurrentEori" should {
    "retrieve the documents when documents are present in the DB" in new Setup {
      when(mockHistDocReqSearchCache.retrieveDocumentsForCurrentEori(any))
        .thenReturn(Future.successful(Seq(histDocRequestSearch, histDocRequestSearch)))

      val service: HistoricDocumentRequestSearchCacheService =
        app.injector.instanceOf[HistoricDocumentRequestSearchCacheService]

      service.retrieveHistDocRequestSearchDocsForCurrentEori("test_eori").map { records =>
        records mustBe Seq(histDocRequestSearch, histDocRequestSearch)
      }
    }

    "return the empty documents when documents are not present in the DB" in new Setup {
      when(mockHistDocReqSearchCache.retrieveDocumentsForCurrentEori(any)).thenReturn(Future.successful(Seq()))
  ...
  "retrieveHistDocRequestSearchDocForStatementReqId" should {
    "retrieve the document when document is present in the DB" in new Setup {
      when(mockHistDocReqSearchCache.retrieveDocumentForStatementRequestID(any))
        .thenReturn(Future.successful(Option(histDocRequestSearch)))

      val service: HistoricDocumentRequestSearchCacheService =
        app.injector.instanceOf[HistoricDocumentRequestSearchCacheService]

      service.retrieveHistDocRequestSearchDocForStatementReqId("5b89895-f0da-4472-af5a-d84d340e7mn5").map { record =>
        record.get mustBe histDocRequestSearch
      }
    }
  ...
  "updateSearchRequestForStatementRequestId" should {
    "update the doc correctly" in new Setup {
      ...
      val updatedSearchRequests: Set[SearchRequest] = searchRequests.map { sr =>
      ...
      when(mockHistDocReqSearchCache.updateSearchRequestForStatementRequestId(any, any))
        .thenReturn(Future.successful(Option(histDocRequestSearch.copy(searchRequests = updatedSearchRequests))))

      val service: HistoricDocumentRequestSearchCacheService =
        app.injector.instanceOf[HistoricDocumentRequestSearchCacheService]

      service.updateSearchRequestForStatementRequestId(histDocRequestSearch, statReqId, searchFailureReasonCode).map {
        optDoc =>
          val doc       = optDoc.get
      ...
      verify(mockHistDocReqSearchCache, times(1))
        .updateSearchRequestForStatementRequestId(updatedSearchRequests, searchID.toString)
    }
  }
  ...
  "updateResultsFoundStatusToNoIfEligible" should {
    "update the resultsFound to no if all the search requests have no for searchSuccessful field" in new Setup {
      ...
      when(
        mockHistDocReqSearchCache.updateResultsFoundStatus(
      ...
      service.updateResultsFoundStatusToNoIfEligible(histDocSearchWithAllSearchRequestsProcessed).map { optDoc =>
        val doc = optDoc.get
      ...
      verify(mockHistDocReqSearchCache, times(1))
        .updateResultsFoundStatus(histDocSearchWithAllSearchRequestsProcessed.searchID.toString, SearchResultStatus.no)
    }
    ...
    "not update the resultsFound if all the search requests do not have no for searchSuccessful field" in new Setup {
      ...
      service.updateResultsFoundStatusToNoIfEligible(histDocRequestSearch).map { optDoc =>
        val doc = optDoc.get
      ...
      " searchSuccessful field" in new Setup {
        ...
        val searchRequestsOb: Set[SearchRequest] = Set(
          ...
          SearchRequest(
            ...
            SearchResultStatus.no,
            Utils.dateTimeAsIso8601(LocalDateTime.now),
            "AWSUnreachable",
            0
          )
        )
        ...
        service
          .updateResultsFoundStatusToNoIfEligible(histDocRequestSearch.copy(searchRequests = searchRequestsOb))
          .map { optDoc =>
            val doc = optDoc.get
  ...
  "processSDESNotificationForStatReqId" should {
    "update the doc correctly" in new Setup {
      ...
      val updatedSearchRequests: Set[SearchRequest] = searchRequests.map { sr =>
        if (sr.statementRequestId.equals(statReqId)) {
          sr.copy(searchSuccessful = SearchResultStatus.yes, searchDateTime = searchDtTime)
        } else {
          sr
        }
      }

      when(mockHistDocReqSearchCache.updateSearchReqsAndResultsFoundStatus(any, any, any)).thenReturn(
        Future.successful(
          Option(
            histDocRequestSearch.copy(
              resultsFound = SearchResultStatus.yes,
      ...
      service.processSDESNotificationForStatReqId(histDocRequestSearch, statReqId).map { optDoc =>
        val doc       = optDoc.get
        val updatedSR = doc.searchRequests.find(x => x.statementRequestId == statReqId).get
      ...
      verify(mockHistDocReqSearchCache, times(1))
        .updateSearchReqsAndResultsFoundStatus(searchID.toString, updatedSearchRequests, SearchResultStatus.yes)
    }
  }
  ...
  "updateSearchRequestRetryCount" should {
    "update the doc correctly" in new Setup {
      ...
      val updatedSearchRequests: Set[SearchRequest] = searchRequests.map { sr =>
        if (sr.statementRequestId.equals(statReqId)) {
          sr.copy(searchFailureReasonCode = searchFailureReasonCode, failureRetryCount = sr.failureRetryCount + 1)
        } else {
          sr
        }
      }

      when(mockHistDocReqSearchCache.updateSearchRequestForStatementRequestId(updatedSearchRequests, searchID.toString))
        .thenReturn(Future.successful(Option(histDocRequestSearch.copy(searchRequests = updatedSearchRequests))))
