...
class EmailVerifiedResponseSpec extends SpecBase {
  ...
  trait Setup {
    val emailVerifiedResOb: EmailVerifiedResponse     = EmailVerifiedResponse(Some(EmailAddress(TEST_EMAIL)))
    val emailUnverifiedResOb: EmailUnverifiedResponse = EmailUnverifiedResponse(Some(EmailAddress(TEST_EMAIL)))
