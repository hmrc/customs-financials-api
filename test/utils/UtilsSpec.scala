  ...
  "emptyString" should {
    ...
    "httpDateFormatter" should {
      "format date according to RFC 7231 format" in {
        ...
        val httpFormatedDate    = httpDateFormatter.format(testDateTime)
        val simpleFormattedDate = simpleFormatter.format(testDateTime)

        httpFormatedDate mustBe simpleFormattedDate
      }
